...
posterior_rsv = as.data.frame(fit_scotland_SEIRS_rsv)[,c("S0", "E0", "I0", 
                                                     "rho", "b", "a", "p")]
...
# mpv pars
posterior_mpv = as.data.frame(fit_scotland_SEIRS_mpv)[,c("S0", "E0","I0",
                                                     "rho", "b", "a", "p")]
...
n_samp <- 5000
samp_ind <- sample(1:nrow(posterior_mpv), n_samp)
posterior_rsv <- posterior_rsv[samp_ind, ]
posterior_mpv <- posterior_mpv[samp_ind, ]
...
npi_scalar2 <- unique(round(sort(c(seq(0.4, 1.1, 0.1), seq(0.6, 0.8, 0.05))),3))
...
# data.frame(x = npi_scalar2, y = npi_nll, mod = "independent") %>% 
#   bind_rows(coupled_nll %>% mutate(mod = "coupled")) %>% # from simulate-postpand_scotland_SEIRS.R
#   ggplot(aes(x = x, y = y, color = mod)) + 
#   geom_point() + 
#   geom_line() + 
#   labs(x = "mobility scalar", y = "negative log likelihood (100K)") + 
#   scale_color_brewer(palette = "Paired") + 
#   theme_bw() + 
#   theme(panel.grid.minor = element_blank(), 
#         legend.position = "bottom", 
#         legend.title = element_blank())
# ggsave("figures/nll_comparison.pdf", width = 4, height = 4)
...
npi_scaled <- scotland_by_wk_full %>%
  ...
  pull(mean_mob)
postpand_sims_ind <- SEIRS_force_discrete_both(
  N = length(npi_scaled), parms_rsv = posterior_rsv, parms_mpv = posterior_mpv,
  pop = scotland_N, npi = npi_scaled,
  testing_scalar_mpv = scotland_by_wk_full %>% filter(pathogen == "mpv") %>% pull(n_tests_ma_scaled),
  testing_scalar_rsv = scotland_by_wk_full %>% filter(pathogen == "rsv") %>% pull(n_tests_ma_scaled),
...
postpand_sims_ind <- postpand_sims_ind %>%
  mutate(draw_id = samp_ind[draw_id])
