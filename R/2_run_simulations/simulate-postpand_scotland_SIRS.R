...
posterior_rsv = as.data.frame(fit_scotland_SIRS)[,c("S0_rsv", "E0_rsv", "I0_rsv", 
                                                     "rho_rsv", "b", "a", "p")]
colnames(posterior_rsv) = c("S0", "E0", "I0", "rho", "b", "a", "p")
posterior_rsv = cbind(posterior_rsv, r = 1, omega = scotland_omega,
                      gamma = 1/(1/scotland_sigma + 1/scotland_gamma), mu = scotland_birthrate)

# mpv pars
posterior_mpv = as.data.frame(fit_scotland_SIRS)[,c("S0_mpv", "E0_mpv","I0_mpv",
                                                     "rho_mpv", "b", "a", "p", 
                                                     "r", "c")]
colnames(posterior_mpv) = c("S0", "E0", "I0", "rho", "b", "a", "p", "r", "c")
posterior_mpv = cbind(posterior_mpv, omega = scotland_omega,
                      gamma = 1/(1/scotland_sigma + 1/scotland_gamma), mu = scotland_birthrate)
...
npi_scaled <- scotland_by_wk_full %>%
  ...
  pull(mean_mob)
postpand_sims <- SIRS_force_discrete_both(
  N = length(npi_scaled), parms_rsv = posterior_rsv, parms_mpv = posterior_mpv,
  pop = scotland_N, npi = npi_scaled,
  testing_scalar_mpv = scotland_by_wk_full %>% filter(pathogen == "mpv") %>% pull(n_tests_ma_scaled),
  testing_scalar_rsv = scotland_by_wk_full %>% filter(pathogen == "rsv") %>% pull(n_tests_ma_scaled),
...
postpand_sims <- postpand_sims %>%
  mutate(npi_scalar = npi_scalars[npi_id], 
         draw_id = samp[draw_id])

# saveRDS(postpand_sims, "data/derived_data/scotland/postpand-sims_scotland_SIRS.rds")