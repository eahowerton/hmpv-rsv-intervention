...
# load simulations
# postpand_sims <- readRDS("data/derived_data/scotland/postpand-sims_scotland_SIRS.rds")
source("R/2_run_simulations/simulate-postpand_scotland_SIRS.R")
...
postpand_sims %>% 
  ...
  geom_line(data = df_plot_npis %>% 
              ...
              mutate(facet_var = factor("S-rsv", levels = facet_var_lvls), 
                     npi_plot = 3.5e6-4e5 + mean_mob*4e5),
  ...
  geom_line(data = df_plot_npis %>% 
              ...
              mutate(facet_var = factor("S-mpv", levels = facet_var_lvls), 
                     npi_plot = 3.5e6-4e5 + mean_mob*4e5),
            aes(y = npi_plot), color = "darkgray") + 
  geom_text(data = data.frame(y = 3.5e6 + 2e5, 
...
ggsave("figures/full-timeseries_SIRS_scotland.pdf", width = 10, height = 6)
