#' function to simulate forced SIERS model 
...
SEIRS_force_discrete <- function(N, parms, pop, npi = 1, rsvforce = 1, testing_scalar){
  ...
  with(as.list(c(parms)),{
    ...
    E <- matrix(NA, nrow = N, ncol = nrow(parms))
    ...
    for (i in 2:N) {
      ...
      # E
      Eoutall = (1 - exp(-parms[,"sigma"] - parms[,"mu"]))*E[i-1,] 
      Edeath = Eoutall*(parms[,"mu"]/(parms[,"mu"] + parms[,"sigma"]))
      Eout = Eoutall - Edeath 
      # I
      ...
      Ipred[i, ] = Eout * parms[,"rho"] * testing_scalar[i] 
    }
    return(bind_rows(melt(S) %>% mutate(variable = "S"), 
                     melt(E) %>% mutate(variable = "E"), 
...
#' @param k integer number of prior weeks to use in rsv forcing, k = 0 implies only current week
...
SEIRS_force_discrete_both = function(N, parms_rsv, parms_mpv, pop, k = 0, 
  ...
  if(k == 1){rsv_force_sim = rsv_I/rsv_max}
  if (k > 1){
    rsv_force_sim = t(cbind(
      # weeks before a full lag is available
      sapply(1:(k-1), function(i){
        if(i == 1){return(t(rsv_I)[,i]/rsv_max)};
        if(i > 1){apply((t(rsv_I)/((i)*rsv_max))[,1:i], 1, sum)}}), 
      # once a full lag is available, roll_sum
      t(apply(t(rsv_I)/(k*rsv_max), 1, roll_sum, n = k))))
  }
  mpv_sim = SEIRS_force_discrete(N = N, parms = parms_mpv, pop = pop, 
                                 npi = npi, rsvforce = rsv_force_sim, 
                                 testing_scalar = testing_scalar_mpv)
  mpv_sim = bind_rows(
    mpv_sim, 
    melt(rsv_force_sim) %>% 
      rename(t = Var1, draw_id = Var2) %>% 
      mutate(variable = "rsv_force"))
  return(
    bind_rows(
