data {
  ...
  // int wk_indx[N];                // weekly index denoting week of year 
  // real wkly_tests_rsv[52];
  // real wkly_tests_mpv[52];
...
parameters {
  ...
  real<lower=0, upper=1> E0_mpv;
  real<lower=0, upper=1> I0_mpv;
  real<lower=0, upper=1> S0_rsv;
  real<lower=0, upper=1> E0_rsv;
  ...
  // real<lower=0, upper=1> a_rho_rsv;       // amplitude of seasonal forcing
  // real<lower=0, upper=1> p_rho_rsv;       // phase of seasonal forcing    
  // real<lower=0, upper=1> a_rho_mpv;       // amplitude of seasonal forcing
  // real<lower=0, upper=1> p_rho_mpv;       // phase of seasonal forcing    
...
transformed parameters {
  ...
  E_mpv[1] = E0_mpv * pop;
  I_mpv[1] = I0_mpv * pop;
  Ipred_mpv[1] = I0_mpv * pop * rho_mpv * tests_mpv[1]; //*  wkly_tests_mpv[wk_indx[1]] 
  R_mpv[1] = (1-S0_mpv-E0_mpv-I0_mpv) * pop;
  
  S_rsv[1] = S0_rsv * pop;
  E_rsv[1] = E0_rsv * pop;
  I_rsv[1] = I0_rsv * pop;
  Ipred_rsv[1] = I0_rsv * pop * rho_rsv * tests_rsv[1]; //*  wkly_tests_rsv[wk_indx[1]] 
  ...
  for(i in 2:N){
    ...
    Eout_rsv = (1-exp(-sigma - mu))*E_rsv[i-1]*(sigma/(mu + sigma));
    ...
    Edeath_rsv = (1-exp(-sigma - mu))*E_rsv[i-1]*(mu/(mu + sigma));
    ...
    Ipred_rsv[i] = Eout_rsv * rho_rsv * tests_rsv[i]; // use incidence (Eout)   * wkly_tests_rsv[wk_indx[i]]
  ...
  for (i in 2:N) {
    ...
    Eout_mpv = (1-exp(-sigma - mu))*E_mpv[i-1]*(sigma/(mu + sigma));
    ...
    Edeath_mpv = (1-exp(-sigma - mu))*E_mpv[i-1]*(mu/(mu + sigma));
    ...
    Ipred_mpv[i] = Eout_mpv * rho_mpv * tests_mpv[i]; // use incidence (Eout)  *  wkly_tests_mpv[wk_indx[i]]
...
model {
  ...
  rho_mpv ~ beta(1, 99);
  rho_rsv ~ beta(1, 99);
  // a_rho_rsv ~ normal(0.5, 0.1);
  // p_rho_rsv ~ normal(0.5, 0.1);
  // a_rho_mpv ~ normal(0.5, 0.1);
  // p_rho_mpv ~ normal(0.5, 0.1);
